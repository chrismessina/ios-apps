...
export async function scrapeAppStoreScreenshots(app: AppDetails): Promise<ScreenshotInfo[]> {
  try {
      ...
      if (srcsetMatch && srcsetMatch[1]) {
        ...
        if (highResUrl && isAppStoreScreenshot(highResUrl)) {
          ...
          if (!screenshotsMap.has(normalizedUrl)) {
            ...
            const contextHtml = html.substring(Math.max(0, artworkMatch.index - 500), 
    ...
    console.log(`[Scraper] Found ${reindexedScreenshots.length} unique screenshots (filtered from ${uniqueScreenshots.length} total)`);
 ...
 */
function addUniqueScreenshots(
  screenshotsMap: Map<string, ScreenshotInfo>,
  newScreenshots: ScreenshotInfo[]
): void {
